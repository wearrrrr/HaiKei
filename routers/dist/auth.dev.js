...
router.post('/signup', function (req, res, next) {
  if (req.body.password === req.body.confirm_password) {
    ...
    crypto.pbkdf2(req.body.password, salt, 310000, 32, 'sha256', function (err, hashedPassword) {
      if (err) {
        return next(res.render('error.ejs', {
          loginState: 'false',
          errCode: "ERR 500: Try again later!"
        }));
      }

      db.run('INSERT INTO users (email, email_verified, username, hashed_password, salt) VALUES (?, ?, ?, ?, ?)', [req.body.email, "false", req.body.username, hashedPassword, salt], function (err) {
        if (err) {
          return next(res.render('error.ejs', {
            loginState: 'false',
