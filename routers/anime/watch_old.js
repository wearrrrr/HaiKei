...
try { 
    ...
    let resx = await axios.get(`${consumetURL}anime/gogoanime/watch/${epData[0]}-episode-${epData[1]}?server=gogocdn`)
    let showDetails = await axios.get(`${consumetURL}anime/gogoanime/info/${epData[0]}`)
    let showInfo = await showDetails.data
    let info = await resx.data;
    return res.render('watch.ejs', {data:info, apiUrl:consumetURL, epData: epData});
    } catch(e) {
